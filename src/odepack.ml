...
let lsoda ?(rtol=1e-6) ?(atol=1e-6) ?atol_vec f t0 y0 tout =
  ...
  let itol, atol = match atol_vec with
    | Some v ->
      ...
      2, v
    | None ->
      let v = Array1.create float64 (Array1.layout y0) 1 in
      v.{1} <- atol;
      1, v in
  (* For now, JT = 2 *)
  ...
  let ydot = set_layout (Array1.sub rwork 1 neq) (Array1.layout y0) in
